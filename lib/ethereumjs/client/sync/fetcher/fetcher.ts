...
export abstract class Fetcher<JobTask, JobResult, StorageItem> extends Readable {
  ...
  constructor(options: FetcherOptions) {
    ...
    this.in = new Heap({
      comparBefore: (a: Job<JobTask, JobResult, StorageItem>, b: Job<JobTask, JobResult, StorageItem>) =>
        a.index < b.index,
    });
    this.out = new Heap({
      comparBefore: (a: Job<JobTask, JobResult, StorageItem>, b: Job<JobTask, JobResult, StorageItem>) =>
  ...
  write() {
    ...
    const writer = new Writable({
      ...
      writev: (many: { chunk: StorageItem; encoding: string }[], cb: Function) =>
        _write(
          (<StorageItem[]>[]).concat(...many.map((x: { chunk: StorageItem; encoding: string }) => x.chunk)),
          null,
          cb
        ),
    });
