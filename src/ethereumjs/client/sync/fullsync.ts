...
export class FullSynchronizer extends Synchronizer {
  ...
  constructor(options: SynchronizerOptions) {
    ...
    this.chain.on("updated", async function () {
      ...
      console.log("vmexecution: updated");
      console.log("stateRoot", await self.execution.vm.stateManager.getStateRoot());
      console.log("getHead", (await self.execution.vm.blockchain.getHead())?.header.number);
      console.log("getIteratorHead", (await self.execution.vm.blockchain.getIteratorHead())?.header.number);
      console.log("getLatestBlock", (await self.execution.vm.blockchain.getLatestBlock())?.header.number);
      if (self.running) {
  ...
  best(): Peer | undefined {
    ...
    for (const peer of peers) {
      if (peer.eth?.status) {
        ...
        if ((!best && td.gte(this.chain.blocks.td)) || (best && best.eth && best.eth.status.td.lt(td))) {
          best = peer;
        }
      }
    }
  ...
  async open(): Promise<void> {
    ...
    this.config.logger.info(
      `Latest local block: number=${number} td=${td} hash=${short(hash)} hardfork=${this.config.chainCommon.hardfork()}`
    );
  }
