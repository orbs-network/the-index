...
export class Libp2pPeer extends Peer {
   ...
   */
  async bindProtocols(node: Libp2pNode, peer: PeerId | multiaddr, server?: Libp2pServer): Promise<void> {
    await Promise.all(
      this.protocols.map(async (p) => {
        ...
        try {
          ...
          const peerInfo = peer instanceof PeerId ? `id=${peer.toB58String()}` : `multiaddr=${peer.toString()}`;
