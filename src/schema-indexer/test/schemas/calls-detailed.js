// try to transfer 300m BAT from the BAT ERC20 dev fund every 100,000 blocks
...
class Schema {
  ...
  async onBlock(blockNumber) {
    if (blockNumber % 100000 == 0) {
      if (!(await this.batContract.isDeployed())) return;
      const amount = new BN("300000000000000000000000000"); // 300m BAT
      const to = "0x2222222222222222222222222222222222222222";
      const result = await this.batContract.methods.transfer(to, amount).call({
        from: "0x88e2efac3d2ef957fcd82ec201a506871ad06204", // dev fund address (batFundDeposit)
        fakeFromAccount: true, // no need to fetch this address' true account (ETH balance and nonce)
        detailedResult: true, // detailed result so we can see the logs
