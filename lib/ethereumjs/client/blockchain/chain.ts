...
export class Chain extends EventEmitter {
   ...
   */
  async getBlocks(block: Buffer | BN, max: number = 1, skip: number = 0, reverse: boolean = false): Promise<Block[]> {
    await this.open();
   ...
   */
  async getHeaders(block: Buffer | BN, max: number, skip: number, reverse: boolean): Promise<BlockHeader[]> {
