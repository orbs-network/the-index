...
export class Processor implements IWeb3 {
  public currentBlockNumber: number = 0;
  public latestBlockNumber: number = 0;
  public vm: VM;

  constructor(public data: IData, public perf: Perf) {
  ...
  async run(schema: ISchema) {
    /**/ this.perf.start("all");
    if (schema.onInit) {
      /**/ this.perf.start("init");
      await schema.onInit(this, this.data);
      /**/ this.perf.end("init");
    }
    ...
    for (this.currentBlockNumber = 1; this.currentBlockNumber <= this.latestBlockNumber; this.currentBlockNumber++) {
      if (schema.onBlock) {
        await schema.onBlock(this.currentBlockNumber);
      }
    }
    if (schema.onDone) {
      await schema.onDone();
  ...
  stateManager = new IndexedStateManager(this);
...
class IndexedStateManager implements EthereumJsStateManager {
