...
export class VMExecution extends Execution {
  ...
  async run(): Promise<number> {
    ...
    console.log("vmexecution: run: startHeadBlock", startHeadBlock.header.number.toNumber());
    let canonicalHead = await this.vm.blockchain.getLatestBlock();
    console.log("vmexecution: run: canonicalHead", canonicalHead.header.number.toNumber());
