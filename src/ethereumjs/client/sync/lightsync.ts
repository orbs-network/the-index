  ...
  best(): Peer | undefined {
    ...
    for (const peer of peers) {
      if (peer.les) {
        ...
        if ((!best && td.gte(this.chain.headers.td)) || (best && best.les && best.les.status.headTd.lt(td))) {
          best = peer;
        }
      }
    }
    ...
    this.headerFetcher
      ...
      .on("fetched", (headers: BlockHeader[]) => {
        ...
        this.config.logger.info(
          `Imported headers count=${headers.length} number=${first.toString(10)} hash=${hash} peers=${this.pool.size}`
        );
      });
