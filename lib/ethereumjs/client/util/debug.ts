...
const main = async () => {
  ...
  const block = Block.fromRLPSerializedBlock(Buffer.from('${block.serialize().toString("hex")}', 'hex'), { common })
  ...
  stateManager.setStateRoot(Buffer.from('${(await execution.vm.stateManager.getStateRoot(true)).toString(
    "hex"
  )}', 'hex'))
